...
class Mpd {
     ...
     */
    function __construct($srv = "localhost", $port = 6600, $pwd = NULL) {
        $this->host = $srv;
        $this->port = $port;
        $this->password = $pwd;
    ...
    function SendCommand($cmdStr,$arg1 = "",$arg2 = "") {
        if ( ! $this->connected ) {
            $this->errStr = "mpd->SendCommand() / Error: Not connected\n";
            return NULL;
        } else {
    ...
    function QueueCommand($cmdStr,$arg1 = "",$arg2 = "") {
        if ( ! $this->connected ) {
            $this->errStr = "mpd->QueueCommand() / Error: Not connected\n";
            return NULL;
        } else {
    ...
    function SendCommandQueue() {
        if ( ! $this->connected ) {
            $this->errStr = "mpd->SendCommandQueue() / Error: Not connected\n";
            return NULL;
        } else {
    ...
    function SetVolume($newVol) {
        ...
        if ( $this->_checkCompatibility(MPD_CMD_SETVOL) ) {
            if ( ! is_null($ret = $this->SendCommand(MPD_CMD_SETVOL,$newVol))) $this->status["volume"] = $newVol;
        } else {
            ...
            if ( is_null($this->status["volume"]) ) {
                ...
                if ( ! is_null($ret = $this->SendCommand(MPD_CMD_VOLUME,$modifier))) $this->status["volume"] = $newVol;
            }
        }
        return $ret;
    }
    ...
    function PLMoveTrack($origPos, $newPos) {
        ...
        if ( $origPos < 0 or $origPos > count($this->playlist) ) {
            $this->errStr = "PLMoveTrack(): argument 1 out of range";
            return NULL;
        }
        if ( $newPos < 0 ) $newPos = 0;
        if ( $newPos > count($this->playlist) ) $newPos = count($this->playlist);
     ...
     */
     function SeekPercent($perc) {
         return $this->SeekTo(round($perc * 0.01 * $this->status["current_track"]["length"]));
     }
    ...
    function SeekTo($pos, $track = -1) {
        ...
        if ( $track < 0 ) {
            $track = $this->status["current_track"]["index"];
        }
    ...
    */
    ...
    function _computeVersionValue($verStr) {
        ...
        return ( 10000 * $ver_maj ) + ( 100 * $ver_min ) + ( $ver_rel );
    }
    ...
    */
    function _checkCompatibility($cmd) {
        ...
        if($req_ver_hi == 0 and $req_ver_low == 0) return TRUE;
    ...
    function RefreshInfo() {
        ...
        $x = Array(0, 0);
        if ( ($this->status["state"] == MPD_STATE_PLAYING) || ($this->status["state"] == MPD_STATE_PAUSED) ) {
            $x = explode(":",$status['time']);
        }
        $this->status["current_track"] = Array(
            "index" => $this->status["song"],
            "id" => $this->status["songid"],
            "progress" => $x[0],
            "length" => $x[1],
            "info" => $this->playlist[$this->status["song"]]
            );
